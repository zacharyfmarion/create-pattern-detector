...
edges `MAX_EDGES=700` checkpoint:
...
It warm-started from the previous no-guide-grid close-pair R1 checkpoint, kept
all heads, increased the training edge envelope from `max_edges=300` to
`max_edges=700`, and preserved the close-pair radius-3 junction-offset recipe.
...
```text
apps/web/public/models/cp-detector-v3-dense-edges-max700-20260618/manifest.json
apps/web/public/models/cp-detector-v3-dense-edges-max700-20260618/model.onnx
```
...
- Promoted continuation envelope: `max_edges=700`, `1500` steps, no head
  reinitialization from the prior no-guide-grid close-pair R1.
...
the `MAX_EDGES=700` dense-edge continuation above:
...
- The non-crease suppression failure is essentially fixed: effective recall now
  matches raw recall, and non-crease conflict fell from 54.2% to about 2.0% in
  the current promoted max700 model.
...
- Clean-15 product strict topology improved on the current product pack:
  ...
  extra / `72` merged; promoted max700 dense-edge continuation `0.9623` edge
  F1 / `112` missing / `88` extra / `66` merged.
...
The max700 continuation also improved the hardest BP angle buckets:
...
| All 179, current max700 | 0.6874 | 0.6124 | 0.8812 |
