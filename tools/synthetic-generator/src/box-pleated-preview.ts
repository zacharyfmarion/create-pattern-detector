...
async function main(): Promise<void> {
  ...
  for (let index = 0; index < args.count; index++) {
    ...
    console.log(`[${index + 1}/${args.count}] wrote ${packing.id} (${packing.stats.leaves} leaves, ${packing.stats.ridgeCreases} ridges, ${packing.stats.stretchDevices} stretch devices, ${packing.stats.offGridRidgeCreases} off-grid ridges, noStretches=${packing.constraints.noStretches})`);
  }

  await Bun.write(join(args.out, "index.html"), renderIndex(packings, args.showContours));
  await Bun.write(join(args.out, "summary.json"), JSON.stringify(summary(packings), null, 2) + "\n");
...
function parseArgs(argv: string[]): CliArgs {
  ...
  for (let index = 0; index < argv.length; index++) {
    ...
    if (arg === "--count") args.count = Number(argv[++index]);
      ...
      console.log("Usage: bun run box-pleated-preview -- --count 8 --out /tmp/box-pleated-packings --bp-studio-root /tmp/bp-studio-source [--no-stretches] [--show-contours]");
      process.exit(0);
    } else {
...
function summary(packings: BoxPleatedPacking[]): unknown {
  return {
