...
import { generateTessellationFoldProgram } from "./tessellation-fold-program.ts";
...
export function generateFold(config: GenerationConfig): FOLDFormat {
  ...
  if (config.family === "tessellation-fold-program") return generateTessellationFoldProgram(config);
