...
if [[ "${CP_ARE_YOU_SURE_LEGACY_NO_GUIDE_GRID_SCRIPT:-}" != "are-you-sure" ]]; then
  cat >&2 <<'EOF'
fatal: run_cpline_runpod_v3_no_guide_grid_probe.sh is retired.

This old name previously launched without the radius-3 close-pair offset
settings, producing checkpoints with junction_offset_radius_px=0.0.
...
exec scripts/training/run_cpline_runpod_v3_no_guide_grid_close_pair_probe.sh
