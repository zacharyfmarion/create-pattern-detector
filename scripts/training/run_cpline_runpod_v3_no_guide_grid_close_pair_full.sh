...
export OUTPUT_ROOT="${OUTPUT_ROOT:-checkpoints/runpod_v3_no_guide_grid_close_pair_full_r1}"
...
export REQUIRE_CLOSE_PAIR_OFFSETS="${REQUIRE_CLOSE_PAIR_OFFSETS:-1}"
export REQUIRE_NO_GUIDE_GRID_PROFILE="${REQUIRE_NO_GUIDE_GRID_PROFILE:-1}"
export REQUIRE_R1_INIT="${REQUIRE_R1_INIT:-1}"
...
verify_run_config() {
  ...
  "$PYTHON" scripts/training/verify_cpline_run_config.py \
    ...
    --expect-suffix "loaded_checkpoint=checkpoints/r1_close_pair_warmstart/latest.pt"
}
