...
- Region candidates now emit pleat lines along each corridor rectangle's long axis. The earlier perpendicular crossbar emission made the diagrams look unlike designer BP pleat runs and produced many artificial dangling endpoints. Long-axis pleats better match the "long alternating M/V strip" visual grammar of BP designs.
...
- Region candidates now add two conservative bounded lane-continuation passes before turn closure:
  ...
  - `terminal-closure` extends a degree-1 pleat endpoint to the sheet border only when the endpoint and the whole extension segment remain inside the relevant BP Studio flap allocation circle. This represents a terminal fan/border continuation, not a sheet-wide sweep.
- Region candidates add a bounded turn-closure pass for active Kawasaki failures caused by simple elbows and T-junctions: perpendicular elbows receive a two-diagonal fan on the opposite side of the turn, and three-way T junctions receive the missing fourth cardinal spoke. This is still an intermediate scaffold completion step, but it removes artificial Kawasaki geometry failures before terminal/hub endpoint closure is attempted.
